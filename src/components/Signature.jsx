...
// Definición del componente funcional Signature
const Signature = () => {
  // useEffect se utiliza para ejecutar código cuando el componente se monta
  useEffect(() => {
    // Función para inicializar el efecto de la firma
    const initializeSignature = () => {
      // Selecciona todos los elementos SVG con la clase 'signature'
      document.querySelectorAll('.signature svg').forEach(svg => {
        // Selecciona todos los elementos 'path', 'circle' y 'rect' dentro del SVG
        const paths = Array.from(svg.querySelectorAll('path, circle, rect'));
        let delay = 0;
        
        // Para cada elemento, calcula la longitud del trazo y configura atributos
        paths.forEach(path => {
          const length = path.getTotalLength();
          const previousStrokeLength = path.dataset.speed || 0;
          const speed = length < 100 ? 20 : Math.floor(length);
          
          delay += parseInt(previousStrokeLength) + 100;
          
          // Configura atributos del path para la animación
    ...
    // Función para animar la firma
    const animateSignature = () => {
      document.querySelectorAll('.signature svg').forEach(svg => {
        const paths = Array.from(svg.querySelectorAll('path, circle, rect'));
        
        // Para cada elemento, configura atributos para la animación
        paths.forEach(path => {
    ...
    // Inicializa la firma al montar el componente
    initializeSignature();

    // Agrega un evento de clic al botón para reiniciar y animar la firma
  ...
  }, []); // El array vacío significa que este efecto se ejecuta solo al montar el componente

  // Renderiza el componente, con un botón para reiniciar la animación
  return (
    <>
...
// Exporta el componente Signature para su uso
